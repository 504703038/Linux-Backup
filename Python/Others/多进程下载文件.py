...
class DownloadThread(Process):
    def __init__(self, thread_name, url, output_path, file_nam, ranges, progess):
        super().__init__()
    ...
    def run(self):
        ...
        while data:
            ...
            data = urlHandler.read(self.one_time_size)
            self.progess[self.name] = self.downloaded
...
def download_file(url, output_path, blocks=8):
    ...
    with Manager() as manager:
        progess = manager.dict()
        threads = []
        for i in range(blocks):
            task = DownloadThread(
                thread_name[i], url, output_path,
                tmpfile_name[i], ranges[i], progess)
            task.start()
            threads.append(task)
        time.sleep(2)
        while islive(threads):
            downloaded = 0
            for i in range(blocks):
                downloaded += progess[thread_name[i]]
